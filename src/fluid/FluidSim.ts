 ...
 * 구버전 ENSIL 의 src/fluid/FluidSim.ts 를 그대로 옮긴 것 (원형 솔리드 커서는 두고 유체만).
 * 참고: PavelDoGreat/WebGL-Fluid-Simulation (MIT). 웹 최적화 선택:
 *  - 속도장 128px / 염료 512px 저해상도 오프스크린 → 화면 크기와 무관한 고정 비용
 *  - half-float 텍스처, WebGL2 필수 (미지원이면 조용히 비활성 — supported=false)
 *  - 압력 야코비 18회, 소용돌이 보강(vorticity confinement)은 꺼 둠(액체 룩)
 *  - 프레임 루프는 여기 없다 — fluid-layer.js 가 돌리고, 유휴 시 완전히 멈춘다
 *
 * 이 프로젝트에서 바꾼 것: display 패스에 uTint 를 넣어 정규화된 염료색의 밝기를 낮출 수 있게 했다.
 * (display 는 최대 채널을 1 로 정규화하므로, 그대로면 항상 형광색이 된다.)
 */

// 레퍼런스 패널 값 반영: sim 128 / density diffusion 1.8 / velocity diffusion 2 /
// pressure 1 / vorticity 0 / splat radius 0.41 / shading on, bloom off
const SIM_RES = 128;
const DYE_RES = 512;
...
uniform vec3 uTint; /* 정규화된 염료색에 곱하는 밝기 — 흰 바닥 위 최종 색을 정한다 */
void main () {
  vec3 c = texture2D(uTexture, vUv).rgb;
  ...
  gl_FragColor = vec4(body * uTint * alpha, alpha); /* premultiplied */
}`;
...
export interface FluidSimOptions {
  edgeLow?: number;
  edgeHigh?: number;
  /** 정규화된 염료색에 곱해지는 밝기 (기본 1,1,1 = ENSIL 원본) */
...
export class FluidSim {
  ...
  constructor(canvas: HTMLCanvasElement, opts: FluidSimOptions = {}) {
    this.canvas = canvas;
  ...
  splat(input: SplatInput) {
    ...
    // 무속도 스플랫(염료 충전용)은 속도 패스를 건너뛴다 — 고임(pool) 비용 절반
  ...
  render() {
    ...
    gl.uniform3f(display.uniforms.uTint, this.tint[0], this.tint[1], this.tint[2]);
  ...
  dispose() {
    // loseContext()는 쓰지 않는다 — 같은 캔버스의 컨텍스트를 다시 받으면(잃은 컨텍스트 반환) 유체가
    // 조용히 죽고, 헤드리스에선 GPU 채널까지 무너진다.
    if (!this.supported) return;
    const gl = this.gl;
    this.ownedFramebuffers.forEach((fbo) => gl.deleteFramebuffer(fbo));
    this.ownedTextures.forEach((texture) => gl.deleteTexture(texture));
