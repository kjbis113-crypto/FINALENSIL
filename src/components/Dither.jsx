...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(invalidate, 1000 / Math.max(1, frameRate));
    return () => window.clearInterval(interval);
  }, [disableAnimation, frameRate, invalidate]);
  ...
  useFrame(({ clock }) => {
    ...
    if (!previousWaveColor.current.every((value, index) => value === waveColor[index])) {
      uniforms.waveColor.value.setRGB(...waveColor);
      previousWaveColor.current = [...waveColor];
    }
    if (
      !previousBackgroundColor.current.every(
